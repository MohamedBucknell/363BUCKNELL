...
class myCheckSum:
    ...
    def run_server(self, outdir, addr, mtu):
        ...
        b=0
        while True:
            # wait for some data to arrive AAAAAAAAAAAAAAAAAAAAAAAAAAAA
            data,remote_addr = sock.recvfrom(mtu+30)
            ...
            if in_xfr:
                ...
                if data[:9] == B"///END\\\\\\":
                    ...
                    # Now extract the checksum
                   
                    # just write the data... except last thing but that did not work!!!
                    #print("Checksum is "+str(data[0:2]))
                    checksum= sum(data[2:])
                    

                    if int(checksum)%100 == int(data[0:2]):
                        g+=1
                        print("GOOD CHECK")
                        ...
                        print("HERE IS A BAD CHECK BBBBBBBBBBBBBBAAAAAAAAAAAAAAAAAADDDDDDDDDDDDDDDDDDDDDDDDDDDDDDD")
    ...
    def chunk(self, payload, mtu):
        ...
        chunks = math.ceil(10*len(payload) / mtu)
